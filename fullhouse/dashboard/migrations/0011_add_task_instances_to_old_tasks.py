...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # for every task that already exists, add a task instance for it
        if not db.dry_run:
            for task in orm.Task.objects.all():
                self._create_task_instance(orm, task)

    def backwards(self, orm):
        # since we're not making any schema changes, we can just pass on a
        # backwards migration
        pass

    def _create_task_instance(self, orm, task):
        current = orm.TaskInstance.objects.create(
            task=task,
